...
var StateQueueManager = (function () {
    function StateQueueManager(states, $registry, builder, $urlRouterProvider, listeners) {
        this.states = states;
    ...
    StateQueueManager.prototype.attachRoute = function ($state, state) {
        ...
        $urlRouterProvider.when(state.url, ['$match', '$stateParams', function ($match, $stateParams) {
                function matchedParamsEqual() {
                    var schema = state.parameters({ inherit: true, matchingKeys: $match });
                    return param_1.Param.equals(schema, param_1.Param.values(schema, $match), $stateParams);
                }
